...
const IOPortMap ioports[] = {
    ...
    { "ioports.car.connected", "CAR_Connected",            IOPORT_READ,                 IOPORT_TYPE_BOOLEAN },
    ...
    { "ioports.mem.connected", "MEM_Connected",            IOPORT_READ,                 IOPORT_TYPE_BOOLEAN },
...
int  try_emit_table_set_intrinsic (ASTNode *table_expr, ASTNode *key_expr, ASTNode *val_node) {
    ...
    // written: a later, separate assignment to hotX/hotY (the documented
    // way to ask for a different hotspot) still emits its own OUT and
    // simply overwrites the port afterward, since codegen is linear in
