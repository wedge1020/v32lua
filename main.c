...
void emit_runtime_library(void) {
    ...
    // (Removed the __heap_pointer ROM allocation here)

    printf("__builtin_strcat:\n");
    printf("  PUSH BP\n  MOV BP, SP\n");
    printf("  MOV R2, [0] ; Read heap pointer\n  PUSH R2\n");
    printf("  MOV R0, [BP+3]\n");
    printf("__strcat_copy1:\n");
    printf("  MOV R1, [R0]\n  MOV R3, 0\n  IEQ R1, R3\n  JT R1, __strcat_phase2\n");
    printf("  MOV [R2], [R0]\n  ADD R0, 1\n  ADD R2, 1\n  JMP __strcat_copy1\n");
    printf("__strcat_phase2:\n");
    printf("  MOV R0, [BP+2]\n");
    printf("__strcat_copy2:\n");
    printf("  MOV R1, [R0]\n  MOV R3, 0\n  IEQ R1, R3\n  JT R1, __strcat_finalize\n");
    printf("  MOV [R2], [R0]\n  ADD R0, 1\n  ADD R2, 1\n  JMP __strcat_copy2\n");
    printf("__strcat_finalize:\n");
    printf("  MOV R1, 0\n  MOV [R2], R1\n  ADD R2, 1\n");
    printf("  MOV [0], R2 ; Update heap pointer\n  POP R0\n");
    printf("  MOV SP, BP\n  POP BP\n  RET\n\n");

    printf("__builtin_table_get:\n");
    printf("  PUSH BP\n  MOV BP, SP\n  MOV R2, [BP+2]\n  MOV R2, [R2]\n");
    printf("__table_get_loop:\n");
    printf("  MOV R1, R2\n  MOV R3, 0\n  IEQ R1, R3\n  JT R1, __table_get_nil\n");
    ...
    printf("  MOV R0, [0] ; Read heap pointer\n  MOV R1, [BP+3]\n  MOV [R0], R1\n");
    printf("  MOV R1, [BP+4]\n  MOV [R0+1], R1\n  MOV R1, [BP+2]\n  MOV R2, [R1]\n");
    printf("  MOV [R0+2], R2\n  MOV [R1], R0\n  MOV R1, [0]\n");
    printf("  ADD R1, 3\n  MOV [0], R1 ; Update heap pointer\n");
